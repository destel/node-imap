...
function _deepEqual(actual, expected) {
  ...
  //7.5. Arrays sort them and compare elemet-wise, Luke  
    ...
    var n = actual.length
    var actualClone = actual.slice(0, n)
    var expectedClone = expected.slice(0, n)

    actualClone.sort(_deepCompare)
    expectedClone.sort(_deepCompare)


    for (var i=0; i<n; i++) {
      if (!_deepEqual(actualClone[i], expectedClone[i])) {
        return false
      }
    }
